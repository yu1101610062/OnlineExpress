...
public class CourierDaoMysql implements BaseCourierDao {
    ...
    public static final String SQL_LOGIN_TIME =
            "UPDATE ExpressOnline.courier SET loginTime = NOW() WHERE phoneNumber = ?";
    public static final String SQL_SEND =
            "UPDATE ExpressOnline.courier SET sendExpress = sendExpress+1 WHERE phoneNumber = ?";
     ...
     */
    @Override
    public void sendTimes(String phoneNumber) {
        Connection conn = DruidUtil.getConnection();
        PreparedStatement state = null;
        try {
            state = conn.prepareStatement(SQL_SEND);
            state.setString(1,phoneNumber);
            state.executeUpdate();
        } catch (SQLException throwables) {
            throwables.printStackTrace();
